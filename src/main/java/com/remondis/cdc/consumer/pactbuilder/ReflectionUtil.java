...
class ReflectionUtil {
  ...
  /**
   * @return Returns <code>true</code> if the specified type is a Java Collection type.
   */
   ...
   *         was specified a {@link ReflectionException} is thrown.
   */
  static Collector getCollector(Class<?> collectionType) {
    if (Set.class.isAssignableFrom(collectionType)) {
      ...
      throw ReflectionException.unsupportedCollection(collectionType);
    }
  }
  ...
  /**
   * @return Returns the property type. If the property is a collection, the collection type is returned.
   */
  static Class<?> getTypeOrListType(PropertyDescriptor pd) {
    Class<?> propertyType = pd.getPropertyType();
    if (isCollection(propertyType)) {
      return findGenericTypeFromMethod(pd.getReadMethod(), 0);
    }
    return propertyType;
  }
